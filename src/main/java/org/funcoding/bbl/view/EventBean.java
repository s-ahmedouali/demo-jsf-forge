package org.funcoding.bbl.view;
...
public class EventBean implements Serializable {
	...
	@PersistenceContext(unitName = "demo-persistence-unit", type = PersistenceContextType.EXTENDED)
	...
	private Predicate[] getSearchPredicates(Root<Event> root) {
		...
		String summary = this.example.getSummary();
		if (summary != null && !"".equals(summary)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("summary")),
					'%' + summary.toLowerCase() + '%'));
		}
		Format format = this.example.getFormat();
		if (format != null) {
			predicatesList.add(builder.equal(root.get("format"), format));
		}
		...
		String level = this.example.getLevel();
